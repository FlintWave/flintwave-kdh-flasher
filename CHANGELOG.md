...
- **Maximize and edge-resize for the borderless window.** The title bar gains a maximize/restore button (double-clicking the bar works too). Window managers refuse maximize hints and edge grips for undecorated windows, so both are implemented in-app: maximize is emulated against the display's work area (restore remembers the previous rect), and edge/corner resizing is a manual margin-band implementation (`window_drag.hit_test_edge` / `resize_geometry`, pure and unit-tested) with proper cursors, anchored opposite edges, and min-size clamping that never slides the window. The splitters reflow fluidly throughout.
- **The Log pane never grows wider than Instructions.** The bottom sash clamps at the midpoint on drags and resizes — reading the workflow guidance beats scrollback width.
